...
Dokumen Spesifikasi Sistem, Aturan Entitas Tenaga Kerja, Skema Gaji Custom Mitra, Fitur Ekspor, Skema Database, dan Panduan Implementasi.
...
* **Peringatan Masa Kontrak & Evaluasi (Notification Engine):**
  * Notifikasi otomatis H-30 dan H-14 sebelum masa *Probation* (3 bulan) atau kontrak *PKWT* (3/6/12 bulan) dan Kontrak *Mitra* berakhir.
...
* **Karyawan PKWT (3, 6, 12 Bulan):**
  $$\text{Take Home Pay} = (\text{Gaji Pokok} + \text{Tunjangan} + \text{Lembur}) - \text{PPh 21 TER} - \text{BPJS Kes (1\%)} - \text{BPJS TK (JHT 2\% + JP 1\%)}$$
* **Karyawan Probation (3 Bulan):**
  $$\text{Take Home Pay} = (\text{Gaji Pokok} + \text{Tunjangan} + \text{Lembur}) - \text{PPh 21 TER (Jika Kena Pajak)}$$
  *(Sistem secara otomatis mengabaikan/meniadakan pemotongan BPJS Kesehatan & BPJS TK).*
...
1. **Opsi Komponen Dasar Kompensasi Mitra:**
   ...
   * **Hourly Rate / Jam Kerja:** Pembayaran berbasis total jam kerja pada *timesheet* $(\text{Jam Kerja} \times \text{Tarif/Jam})$.
   * **Daily Rate / Per Hari:** Pembayaran berbasis jumlah hari hadir $(\text{Hari Kerja} \times \text{Tarif/Hari})$.
   ...
   * **Unit / Output-Based:** Pembayaran berdasarkan kuantitas hasil kerja $(\text{Jumlah Unit Selesai} \times \text{Tarif/Unit})$.
...
* **Candidate Pipeline (Kanban Board):** Tahapan seleksi visual (*Applied* $\rightarrow$ *Screening* $\rightarrow$ *Interview* $\rightarrow$ *Offering* $\rightarrow$ *Hired*).
